...
class BrandOfflineCaseList extends Component {
    constructor(props) {
        super(props);
        this.state = {
            pageSize: 10,
    ...
    componentWillMount() {
        let { brandOfflineCaseList, history } = this.props;
        if (!brandOfflineCaseList.length || (brandOfflineCaseList.length && history.action !== 'POP' && !history.location.query)) {
			this.getBarndOfflineCaseList([], 1)
		}else {
    ...
    // //获取数据
    getBarndOfflineCaseList(oldThreadListData, pageNo) {
        let { searchData, pageSize } = this.state
        let { getBarndOfflineCaseList } = this.props
        let data = Object.assign({}, searchData);
        data.pageNo = pageNo
        data.pageSize = pageSize
        getBarndOfflineCaseList(data, oldThreadListData)
    }
    ...
    createPaginationOption() {
        ...
        return {
            ...
            total,
            onChange: (page, pageSize) => this.getBarndOfflineCaseList([], page),
    ...
    renderOperate(record) {
        ...
        if ( record.status === 1 && getButtonPrem(permissionList, '019001003')) {
            return (
                <Link to={`/thread/offlinecaselist/detail?cid=${record.id}&brand=audit`}>
    ...
    renderStateName(record) {
        ...
        return (
            <div>
                ...
                {
                    record.status === 4 && record.quitReason ? (
                        <p>
                            ({record.quitReason})
                        </p>
                    ):''
                ...
                {
                    record.status === 2 && record.auditReason ? (
                        <p>
                            ({record.auditReason})
                        </p>
                    ):''
    ...
    render() {
        let { intl, isFetch, total, lawyerBrand, brandOfflineCaseList, offlineCaseType, offlineCaseClueType, brandOffLineCaseStatusList } = this.props
        ...
        let breadcrumbData = [
            ...
            { link: '', titleId: 'router.brand.offline.case.management', title: '品牌方线下案件管理' },
        ];
        return (
            <Content breadcrumbData={breadcrumbData}>
                        ...
                        <Col span={6}>
                            <Form.Item label={intl.formatMessage({ id: "global.status", defaultMessage: "状态", description: "状态" })}>
                                ...
                                >
                                    <Option value=""><FormattedMessage id="global.all" defaultMessage="全部" description="全部" /></Option>
                                    {
                                        brandOffLineCaseStatusList && brandOffLineCaseStatusList.filter(item => item.isDel === 0)
                ...
                <Table
                    dataSource={brandOfflineCaseList}
...
export default injectIntl(BrandOfflineCaseList)
