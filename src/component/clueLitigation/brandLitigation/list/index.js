...
class Litigation extends Component {
    ...
    componentWillMount() {
        let { brandLigitaionList, history } = this.props;
        if (history.action !== 'POP') {
            if (history.location.query && history.location.query.goBack && brandLigitaionList.length) {
    ...
    renderOperate(record) {
        ...
        if (record.status === 13 && getButtonPrem(permissionList, '019001001')) {
            return (
                <Link to={`/thread/brand/detail?suitNo=${record.suitNo}`}>
    ...
    renderStateName(record) {
        ...
        return (
            <div>
                ...
                {
                    record.status === 1 ? record.acceptReason ?
                        <p>
                            ({record.acceptReason})
                        </p> : '' : ''
                }
    ...
    render() {
        let { intl, isFetch, total, lawyerBrand, brandLigitaionList, ligiationStatus } = this.props
        ...
        let breadcrumbData = [
            ...
            { link: '', titleId: 'router.litigation.brand.case.management', title: '品牌方诉讼案件管理' },
        ];
        return (
