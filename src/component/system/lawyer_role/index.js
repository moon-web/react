...
export default class RoleList extends Component {
    ...
    // 搜索
    handleSearch() {
        let { searchData, roleNameLike,  status } = this.state;
        searchData = {
    ...
    renderOperate(record) {
        ...
        return(
            <div className='roleOperate'>
                ...
                {
                    getButtonPrem(permissionList, '010016004') ? (
                        record.canDelete === 1 ? (
                ...
                {
                    getButtonPrem(permissionList, '010016005') ?
                    <Link className="opreateStyle" to={`/system/new/lawyeMenu?roleId=${record.roleId}`} >
    ...
    roleDisable(roleId,type) {
        let { rolelist, intl, systemRoleStatusType, getLawyerType } = this.props
    ...
    //删除角色操作
    deleteRole(roleId) {
        let { pageNo,editRoleDel,getLawyerType } = this.props
        let data={
            id:roleId
        }
        if(editRoleDel){
    ...
    //Modal确认
    addSystemModalOk() {
        let { intl, roleAdd, getLawyerType } = this.props
    ...
    render() {
        ...
        let { roleNameLike,status, addModalVisible, addSystemRoleData } = this.state
        let breadcrumbData = [
            { link: '/', titleId: 'router.home', title: '首页' },
            { link: '', titleId: 'router.system.management', title: '系统管理' },
            { link: '', titleId: 'router.system.lawyer.role.management', title: '专案账号类型管理' }
        ];
        return (
            <Content breadcrumbData={breadcrumbData}>
                        ...
                        <Col span={6}>
                            <Form.Item label={intl.formatMessage({ id: "system.role.name", defaultMessage: "角色名称", description: "角色名称" })}>
                                <Input onPressEnter={(e) => this.handleSearch()}  placeholder={intl.formatMessage({ id: "system.role.please.enter.role.name", defaultMessage: "请输入角色名称", description: "请输入角色名称" })} 
                                    onChange={e => this.setState({ roleNameLike: e.target.value.trim() })} value={roleNameLike}/>
                            </Form.Item>
                        </Col>
                        <Col span={6}>
				...
				<Row className="operation-btns">
					<Col span={24}>
                        {
                            getButtonPrem(permissionList, '010016006') ?
